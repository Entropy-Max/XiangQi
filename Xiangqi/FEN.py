...
def flip_fen_case(fen):

    return ''.join(ch.lower() if ch.isupper() else ch.upper() if ch.islower() else ch for ch in fen)
...
def draw_fen(fen,orientation='h'):
